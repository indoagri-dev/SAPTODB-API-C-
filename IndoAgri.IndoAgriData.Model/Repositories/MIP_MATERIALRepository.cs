...
{
    public class MIP_MATERIALRepository
    {
         private INDOAGRI_DATAEntities _db;
         public MIP_MATERIALRepository()
        {
            try
            {
        ...
        public IQueryable<ZTM_MATPRC_MIP_TEMP> FindAll() 
        {
            return from a in _db.ZTM_MATPRC_MIP_TEMP select a;
        }

        public void Insert(ZTM_MATPRC_MIP_TEMP assetStat)
        {
            _db.ZTM_MATPRC_MIP_TEMP.Add(assetStat);
        }

        public void Update(ZTM_MATPRC_MIP_TEMP assetStat)
        {
            var asset = (from a in _db.ZTM_MATPRC_MIP_TEMP where a.ID == assetStat.ID select a).Any();
            if (asset)
            {
                _db.ZTM_MATPRC_MIP_TEMP.Attach(assetStat);
