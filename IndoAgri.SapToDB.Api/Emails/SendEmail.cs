...
namespace IndoAgri.SapToDB.Api.Emails
{
    public class SendEmail
    {
        ...
        public TemplateNotifEmail getEmailTemplate(string processName, string modulCode)
        {
            TemplateNotifEmail objTemplate = new TemplateNotifEmail();
          
                objTemplate = _db.TemplateNotifEmails.Where(x => x.tpl_ProcessNameCode == processName
        ...
        public string userEmail(int id)
        {
            ...
            if (true)
            {
                    email = _db.Users.Where(x => x.Status == true && x.Id == id).Select(x => x.Email).FirstOrDefault();
